    ...
    function tick($image, $width, $height)
    {
        ...
        for ($x = 0; $x < $this->columns; $x++) {
            for ($y = 0; $y < $this->rows; $y++) {
                ...
                $offsetX = $startX + ($x * $this->grid[$x][$y]->size);
                $offsetY = $startY + ($y * $this->grid[$x][$y]->size);

                $this->grid[$x][$y]->tick($image, $offsetX, $offsetY);
            }
        }
    }
    ...
    function render($image, $width, $height)
    {
        ...
        for ($x = 0; $x < $this->columns; $x++) {
            for ($y = 0; $y < $this->rows; $y++) {
                ...
                $this->grid[$x][$y]->render($image, $offsetX, $offsetY);
            }
        }
    }
}
